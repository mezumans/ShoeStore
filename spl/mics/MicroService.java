...
import bgu.spl.Impl.MessageBusImpl;
...
public abstract class MicroService implements Runnable {
	...
	private  HashMap<Class<? extends Request> ,Callback> RequestCallbackMap;
	private HashMap<Class<? extends Broadcast> ,Callback>  BroadcastCallbackMap;
	private HashMap<Request,Callback> RequestCompleteCallbackMap;
	protected CountDownLatch latchObject;
	 ...
	 */
	public MicroService(String name,CountDownLatch  latchObject) {
		this.name = name;
		MessageBus= MessageBusImpl.getInstance();
		RequestCallbackMap=new HashMap<Class<? extends Request>,Callback>();
		BroadcastCallbackMap=new HashMap<Class<? extends Broadcast>,Callback>();
		RequestCompleteCallbackMap=new HashMap<Request,Callback>();
	...
	protected final <R extends Request> void subscribeRequest(Class<R> type, Callback<R> callback) {
		//TODO: implement this.
			...
			if(!RequestCallbackMap.containsKey(type)){
				MessageBus.subscribeRequest(type,this);
				RequestCallbackMap.put(type,callback);
			}else logger.info("The request type is already registered");}
	 //true
	...
	protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
			...
			if(!BroadcastCallbackMap.containsKey(type)){
				MessageBus.subscribeBroadcast(type, this);
				BroadcastCallbackMap.put(type,callback);	
			}else logger.warning("The broadcast tye is already registerd");
	} //true 
	...
	protected final <T> boolean sendRequest(Request<T> r, Callback<T> onComplete) {
		if(MessageBus.sendRequest(r, this)){
			RequestCompleteCallbackMap.put(r,onComplete);
			return true;}

		else {logger.warning("the request has not been sent"); 
			return false;} 
	}
	...
	logger.severe(getName()+" terminated");
		...
		while (!terminated) {
			try {
				...
				if(Request.class.isAssignableFrom(message.getClass())){
					RequestCallbackMap.get(message.getClass()).<Request>call(message);	
				}
				else if(Broadcast.class.isAssignableFrom(message.getClass())){
					BroadcastCallbackMap.get(message.getClass()).<Broadcast>call(message);		
				}
				else if(RequestCompleted.class.isAssignableFrom(message.getClass())){
					Object completedRequest = ((RequestCompleted)message).getCompletedRequest();
					Object objectResult = ((RequestCompleted) message).getResult();
					Callback currentCompletedRequest = (Callback) RequestCompleteCallbackMap.get(completedRequest);
					currentCompletedRequest.call(objectResult);
				}
